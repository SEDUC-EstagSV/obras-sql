...
CREATE TABLE IF NOT EXISTS `obra_has_usuario` (
  ...
  CONSTRAINT `fk_obra_has_usuario_obra1` FOREIGN KEY (`cd_Obra`) REFERENCES `obra` (`cd_Obra`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_obra_has_usuario_usuario1` FOREIGN KEY (`cd_Usuario`) REFERENCES `usuario` (`cd_Usuario`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Exportação de dados foi desmarcado.

-- Copiando estrutura para tabela seduc_db.pedido_recuperacao
CREATE TABLE IF NOT EXISTS `pedido_recuperacao` (
  `cd_pedido_recuperacao` int(11) NOT NULL AUTO_INCREMENT,
  `cd_Usuario` int(11) DEFAULT NULL,
  ...
  PRIMARY KEY (`cd_pedido_recuperacao`),
  KEY `fk_pedido_recuperacao_usuario1` (`cd_Usuario`),
  CONSTRAINT `fk_pedido_recuperacao_usuario1` FOREIGN KEY (`cd_Usuario`) REFERENCES `usuario` (`cd_Usuario`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
CREATE TABLE IF NOT EXISTS `usuario` (
  ...
  CONSTRAINT `fk_usuario_fornecedor1` FOREIGN KEY (`cd_Fornecedor`) REFERENCES `fornecedor` (`cd_Fornecedor`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_usuario_nivel_autoridade1` FOREIGN KEY (`cd_nivelAutoridade`) REFERENCES `nivel_autoridade` (`cd_nivelAutoridade`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
-- Copiando estrutura para evento seduc_db.DeletaPedidoRecuperacao
DELIMITER //
CREATE EVENT `DeletaPedidoRecuperacao` ON SCHEDULE EVERY 1 DAY STARTS '2023-02-03 09:00:00' ON COMPLETION PRESERVE ENABLE DO DELETE FROM pedido_recuperacao
	WHERE CAST(dt_pedido_recuperacao AS Date) < CAST(CURRENT_TIMESTAMP() AS DATE)//
DELIMITER ;

-- Copiando estrutura para evento seduc_db.ExpiraPedidoDeRecuperacao
DELIMITER //
CREATE EVENT `ExpiraPedidoDeRecuperacao` ON SCHEDULE EVERY 4 MINUTE STARTS '2023-02-03 11:00:00' ON COMPLETION PRESERVE ENABLE DO UPDATE pedido_recuperacao SET ds_ativo = 0
